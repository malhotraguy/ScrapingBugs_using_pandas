...
for BugId in [214172, 221927]:#range(214000,353001):
    #BugId=215518
    ...
    for j in range(0, len(tr_elements)):
        ...
        col=[]
    if Considered_Bug:
        Field_list = [status_NotReassigned,severity_NotReassigned,version_NotReassigned,product_NotReassigned,os_NotReassigned,priority_NotReassigned,component_NotReassigned,assignee_NotReassigned]
        Status_done = False
        Severity_done = False
        Version_done = False
        Product_done = False
        ...
        for j in range(1, len(tr_elements)):
            ...
            if "status" in col and not(Status_done):
                ...
                if (not (col[col.index("severity") + 1].isspace()) and (col[col.index("severity") + 1].isspace()) != "--"):
                    ...
                    Field_list.remove(severity_NotReassigned)
                ...
                if (not (col[col.index("priority") + 1].isspace()) and (col[col.index("priority") + 1].isspace()) != "--"):
                    ...
                    Field_list.remove(priority_NotReassigned)
                ...
                if (not (col[col.index("component") + 1].isspace()) and (col[col.index("component") + 1].isspace()) != "--"):
                    ...
                    Field_list.remove(component_NotReassigned)
